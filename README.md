...
- **Waypoints** — mark a point total worth returning to, with an optional name (e.g. "Level 20"), and it renders as a labeled divider wherever your training order actually crosses it; click its chip to highlight everything up to that point. Anchored to a point total rather than a list position, so reordering, undo, and Reset Build never invalidate one — Reset Build keeps waypoints the same way it keeps owned progress, and they travel with the plan (named Builds, export text, share links)
