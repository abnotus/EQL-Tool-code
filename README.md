...
- Undocumented per-rank costs *and* effect values (marked `?` on the wiki) can show a pattern-inferred estimate instead — cross-referenced against genuinely comparable AAs (same rank count and matching known costs for costs; explicitly-declared sibling AAs sharing the same effect formula for effects — never a coincidental text match), with a color-coded confidence tier (high/medium/low, plus a separate hand-picked "very low" tier for the handful no comparable AA could cross-check) and a tooltip explaining what it's based on. Shown everywhere a cost or description appears — the tree, side panel, Browse All AAs, Summary, and the Progression tab (including its per-step cost and next-rank preview). Purely a display hint: an estimate never substitutes for the real value in point-spending math or anywhere descriptions are otherwise read (search, export text), and the moment the wiki documents the real value, the estimate is superseded automatically, not by anyone remembering to remove it. If a build has already spent points on a cost like that, the topbar's Points Spent number blends the estimate in (turning blue, `~199` instead of `167`, with the confirmed/estimated breakdown in its tooltip — same hover-to-disclose pattern as every other estimate badge, not a second visible number) rather than just looking unaffected — the Progression tab's own running totals stay real regardless
...
Same feature, extended from per-rank point costs to the numeric values embedded in AA effect descriptions — the `?` in something like `"Increases your critical hit chance by 1/?/5/10%."`. Regenerates `src/effectGuesses.js`, keyed one level deeper than costs (AA → progression index → rank index) since a single description can hold more than one independent progression (Adamant Will's resist-charm and resist-mesmerization percentages are two separate ones in the same sentence).

The one real difference from cost-guessing: sibling matching only ever compares AAs within an explicitly hand-declared group (`EFFECT_SIBLING_GROUPS` in the script) — a human confirming two AAs really share the same underlying formula, not a text-similarity guess at which ones might be comparable. A cost curve recurring across unrelated AAs is a real, useful coincidence (the game reuses cost templates); an effect *magnitude* recurring across two unrelated descriptions isn't evidence of anything. Bounded interpolation and the manual-fallback tier work exactly like the cost version otherwise.
...
### A cost or effect estimate can never outrank a real one, by construction

`src/costGuesses.js` (generated by `wiki-sync/guess_costs.py`, see above) is only ever consulted through `keys.js`'s `costGuessFor` when the real `costs[rankIdx]` is exactly `"?"` — `logic.js`'s `costNum()`/`spentPoints()` never look at it at all, so an estimate can't affect real point totals no matter what. This isn't a convention someone has to remember; it's structural. The moment a real number replaces `"?"` in `data.src.js`, that slot's guess (if the generator even still has one sitting in `costGuesses.js`) is never read again — nothing to flip, nothing to delete first. A regeneration pass will eventually stop emitting the now-unreachable guess entirely, but leaving a stale one in the generated file costs nothing; it's already dead code the moment the real value lands.

`src/effectGuesses.js` carries the identical guarantee one level down: `render.js`'s `highlightRankValue` only ever substitutes a guess for a slot whose text is exactly `"?"` inside the description string itself — search (`aaMatchesQuery`), export text, and everywhere else a description is read all still see the real, unmodified string with its `"?"`s intact. An estimate is purely how one specific rendering pass *displays* that string; the underlying data — and therefore anything that reads it for a purpose other than showing it on screen — never sees anything but what the wiki actually says.
